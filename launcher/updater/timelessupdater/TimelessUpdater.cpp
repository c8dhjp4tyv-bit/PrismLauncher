...
#include "updater/timelessupdater/UpdaterDialogs.h"
...
void appDebugOutput(QtMsgType type, const QMessageLogContext& context, const QString& msg)
{
    ...
    TimelessUpdaterApp* app = static_cast<TimelessUpdaterApp*>(QCoreApplication::instance());
...
TimelessUpdaterApp::TimelessUpdaterApp(int& argc, char** argv) : QApplication(argc, argv)
{
    ...
    parser.setApplicationDescription(QObject::tr("An auto-updater for Timeless Launcher"));
    ...
    if (!dirParam.isEmpty()) {
        // The dir parameter makes the Timeless Launcher data path point to the requested location.
        // on command line
    ...
    m_updateLogPath = FS::PathCombine(m_dataPath, "logs", "timeless_launcher_update.log");
    ...
    QString launcher_executable = FS::PathCombine(binPath, BuildConfig.LAUNCHER_APP_BINARY_NAME);
#if defined Q_OS_WIN32
    launcher_executable.append(".exe");
#endif

    if (!QFileInfo(launcher_executable).isFile()) {
        showFatalErrorMessage(tr("Unsupported Installation"), tr("The updater can not find the main executable."));
    }

    m_launcherExecutable = launcher_executable;

    auto launcher_update_url = parser.value("update-url");
    if (launcher_update_url.isEmpty())
        launcher_update_url = BuildConfig.UPDATER_GITHUB_REPO;

    m_launcherRepoUrl = QUrl::fromUserInput(launcher_update_url);
    ...
    if (!version.isEmpty()) {
        if (version.contains('-')) {
            ...
            m_launcherVersionChannel = version.mid(index + 1);
            version = version.left(index);
        } else {
            m_launcherVersionChannel = "stable";
        }
        auto version_parts = version.split('.');
        m_launcherVersionMajor = version_parts.takeFirst().toInt();
        m_launcherVersionMinor = version_parts.takeFirst().toInt();
        if (!version_parts.isEmpty())
            m_launcherVersionPatch = version_parts.takeFirst().toInt();
        else
            m_launcherVersionPatch = 0;
    }

    m_allowPreRelease = parser.isSet("pre-release");

    auto marker_file_path = QDir(m_rootPath).absoluteFilePath(".timeless_launcher_updater_unpack.marker");
    ...
    if (marker_file.exists()) {
        ...
        QMetaObject::invokeMethod(this, &TimelessUpdaterApp::loadReleaseList, Qt::QueuedConnection);
    }
}

TimelessUpdaterApp::~TimelessUpdaterApp()
{
...
void TimelessUpdaterApp::fail(const QString& reason)
{
...
void TimelessUpdaterApp::abort(const QString& reason)
{
    qCritical() << qPrintable(reason);
    m_status = Aborted;
    exit(2);
...
void TimelessUpdaterApp::showFatalErrorMessage(const QString& title, const QString& content)
{
    m_status = Failed;
...
void TimelessUpdaterApp::run()
{
    qDebug() << "found" << m_releases.length() << "releases on github";
    qDebug() << "loading exe at" << m_launcherExecutable;
    ...
    if (!loadLauncherVersionFromExe(m_launcherExecutable)) {
        m_launcherVersion = BuildConfig.printableVersionString();
        m_launcherVersionMajor = BuildConfig.VERSION_MAJOR;
        m_launcherVersionMinor = BuildConfig.VERSION_MINOR;
        m_launcherVersionPatch = BuildConfig.VERSION_PATCH;
        m_launcherVersionChannel = BuildConfig.VERSION_CHANNEL;
        m_launcherGitCommit = BuildConfig.GIT_COMMIT;
    }
    m_status = Succeeded;

    qDebug() << "Executable reports as:" << m_launcherBinaryName << "version:" << m_launcherVersion;
    qDebug() << "Version major:" << m_launcherVersionMajor;
    qDebug() << "Version minor:" << m_launcherVersionMinor;
    qDebug() << "Version minor:" << m_launcherVersionPatch;
    qDebug() << "Version channel:" << m_launcherVersionChannel;
    qDebug() << "Git Commit:" << m_launcherGitCommit;
    ...
    if (m_isFlatpak) {
        showFatalErrorMessage(tr("Updating flatpack not supported"), tr("Actions outside of checking if an update is available are not "
                                                                        "supported when running the flatpak version of Timeless Launcher."));
        return;
    }
    if (m_isAppimage) {
        bool result = true;
...
void TimelessUpdaterApp::moveAndFinishUpdate(QDir target)
{
    ...
    if (error) {
        ...
        auto fail_marker = FS::PathCombine(m_dataPath, ".timeless_launcher_update.fail");
        FS::copy(m_updateLogPath, fail_marker).overwrite(true)();
    } else {
        logUpdate(tr("Update succeed."));
        auto success_marker = FS::PathCombine(m_dataPath, ".timeless_launcher_update.success");
        FS::copy(m_updateLogPath, success_marker).overwrite(true)();
    }
    auto update_lock_path = FS::PathCombine(m_dataPath, ".timeless_launcher_update.lock");
...
void TimelessUpdaterApp::printReleases()
{
...
QList<GitHubRelease> TimelessUpdaterApp::nonDraftReleases()
{
...
QList<GitHubRelease> TimelessUpdaterApp::newerReleases()
{
    QList<GitHubRelease> newer;
    for (auto rls : nonDraftReleases()) {
        if (rls.version > m_launcherVersion)
            newer.append(rls);
    }
    return newer;
}

GitHubRelease TimelessUpdaterApp::selectRelease()
{
    ...
    SelectReleaseDialog dlg(Version(m_launcherVersion), releases);
...
QList<GitHubReleaseAsset> TimelessUpdaterApp::validReleaseArtifacts(const GitHubRelease& release)
{
...
GitHubReleaseAsset TimelessUpdaterApp::selectAsset(const QList<GitHubReleaseAsset>& assets)
{
...
void TimelessUpdaterApp::performUpdate(const GitHubRelease& release)
{
...
QFileInfo TimelessUpdaterApp::downloadAsset(const GitHubReleaseAsset& asset)
{
...
bool TimelessUpdaterApp::callAppImageUpdate()
{
...
void TimelessUpdaterApp::clearUpdateLog()
{
    FS::deletePath(m_updateLogPath);
}

void TimelessUpdaterApp::logUpdate(const QString& msg)
{
...
void TimelessUpdaterApp::performInstall(QFileInfo file)
{
    ...
    auto changelog_path = FS::PathCombine(m_dataPath, ".timeless_launcher_update.changelog");
    FS::write(changelog_path, m_install_release.body.toUtf8());

    logUpdate(tr("Updating from %1 to %2").arg(m_launcherVersion).arg(m_install_release.tag_name));
    if (m_isPortable || file.fileName().endsWith(".zip") || file.fileName().endsWith(".tar.gz")) {
        write_lock_file(update_lock_path, QDateTime::currentDateTime(), m_launcherVersion, m_install_release.tag_name, m_rootPath, m_dataPath);
...
void TimelessUpdaterApp::unpackAndInstall(QFileInfo archive)
{
    logUpdate(tr("Backing up install"));
    backupAppDir();

    if (auto loc = unpackArchive(archive)) {
        auto marker_file_path = loc.value().absoluteFilePath(".timeless_launcher_updater_unpack.marker");
...
void TimelessUpdaterApp::backupAppDir()
{
    auto manifest_path = FS::PathCombine(m_rootPath, "manifest.txt");
    QFileInfo manifest(manifest_path);
    ...
    if (file_list.isEmpty()) {
        ...
        if (BuildConfig.BUILD_ARTIFACT.toLower().contains("linux")) {
            file_list.append({ "TimelessLauncher", "bin", "share", "lib" });
        } else {  // windows by process of elimination
            file_list.append({
                "jars",
                "timeless-launcher.exe",
                "timeless-launcher_filelink.exe",
                "timeless-launcher_updater.exe",
                "qtlogging.ini",
                "imageformats",
                "iconengines",
                "platforms",
    ...
    auto backup_dir =
        FS::PathCombine(app_dir.absolutePath(),
                        QStringLiteral("backup_") + QString(m_launcherVersion).replace(s_replaceRegex, QString("_")) + "-" + m_launcherGitCommit);
    FS::ensureFolderPathExists(backup_dir);
    auto backup_marker_path = FS::PathCombine(m_dataPath, ".timeless_launcher_update_backup_path.txt");
...
std::optional<QDir> TimelessUpdaterApp::unpackArchive(QFileInfo archive)
{
    auto temp_extract_path = FS::PathCombine(m_dataPath, "timeless_launcher_update_release");
...
bool TimelessUpdaterApp::loadLauncherVersionFromExe(const QString& exe_path)
{
    QProcess proc = QProcess();
    ...
    if (first_parts.length() < 2)
        return false;
    m_launcherBinaryName = first_parts.takeFirst();
    auto version = first_parts.takeFirst().trimmed();
    m_launcherVersion = version;
    if (version.contains('-')) {
        auto index = version.indexOf('-');
        m_launcherVersionChannel = version.mid(index + 1);
        version = version.left(index);
    ...
    if (!version_parts.isEmpty())
        ...
        m_launcherVersionPatch = 0;
    m_launcherGitCommit = lines.takeFirst().simplified();
    return true;
}

void TimelessUpdaterApp::loadReleaseList()
{
    auto github_repo = m_launcherRepoUrl;
...
void TimelessUpdaterApp::downloadReleasePage(const QString& api_url, int page)
{
    int per_page = 30;
    ...
    connect(download.get(), &Net::Request::failed, this, &TimelessUpdaterApp::downloadError);
...
int TimelessUpdaterApp::parseReleasePage(const QByteArray* response)
{
...
GitHubRelease TimelessUpdaterApp::getLatestRelease()
{
...
bool TimelessUpdaterApp::needUpdate(const GitHubRelease& release)
{
    auto current_ver = Version(QString("%1.%2.%3").arg(m_launcherVersionMajor).arg(m_launcherVersionMinor).arg(m_launcherVersionPatch));
    return current_ver < release.version;
}

void TimelessUpdaterApp::downloadError(QString reason)
{
