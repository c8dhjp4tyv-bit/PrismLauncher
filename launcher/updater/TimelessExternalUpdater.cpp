...
#include "TimelessExternalUpdater.h"
...
class TimelessExternalUpdater::Private {
   public:
    QDir appDir;
    QDir dataDir;
    QTimer updateTimer;
...
TimelessExternalUpdater::TimelessExternalUpdater(QWidget* parent, const QString& appDir, const QString& dataDir)
    : priv(new TimelessExternalUpdater::Private())
{
    priv->appDir = QDir(appDir);
    priv->dataDir = QDir(dataDir);
    auto settingsFile = priv->dataDir.absoluteFilePath("timeless_launcher_update.cfg");
...
TimelessExternalUpdater::~TimelessExternalUpdater()
{
...
void TimelessExternalUpdater::checkForUpdates()
{
    checkForUpdates(true);
}

void TimelessExternalUpdater::checkForUpdates(bool triggeredByUser) const
{
...
bool TimelessExternalUpdater::getAutomaticallyChecksForUpdates()
{
    return priv->autoCheck;
}

double TimelessExternalUpdater::getUpdateCheckInterval()
{
    return priv->updateInterval;
}

bool TimelessExternalUpdater::getBetaAllowed()
{
    return priv->allowBeta;
}

void TimelessExternalUpdater::setAutomaticallyChecksForUpdates(bool check)
{
...
void TimelessExternalUpdater::setUpdateCheckInterval(double seconds)
{
...
void TimelessExternalUpdater::setBetaAllowed(bool allowed)
{
...
void TimelessExternalUpdater::resetAutoCheckTimer() const
{
...
void TimelessExternalUpdater::connectTimer()
{
    connect(&priv->updateTimer, &QTimer::timeout, this, &TimelessExternalUpdater::autoCheckTimerFired);
}

void TimelessExternalUpdater::disconnectTimer()
{
    disconnect(&priv->updateTimer, &QTimer::timeout, this, &TimelessExternalUpdater::autoCheckTimerFired);
}

void TimelessExternalUpdater::autoCheckTimerFired() const
{
    qDebug() << "Auto update Timer fired";
    checkForUpdates(false);
}

void TimelessExternalUpdater::offerUpdate(const QString& versionName,
...
void TimelessExternalUpdater::performUpdate(const QString& versionTag) const
{
    QProcess proc;
    auto exeName = QStringLiteral("%1_updater").arg(BuildConfig.LAUNCHER_APP_BINARY_NAME);
#ifdef Q_OS_WIN32
